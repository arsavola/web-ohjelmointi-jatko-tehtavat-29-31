...
module.exports = 
{
    fetchAll: function(req, res){
      ...
      var valmistaja = req.query.valmistaja;
      var tuotetyyppi = req.query.tuotetyyppi;
      // var sql = "SELECT * FROM tuote WHERE 1=1";
      var sql = "SELECT tuote.nimi as nimi, tuote.valmistaja, tuote.hinta, tuotetyyppi.nimi as selite FROM tuote INNER JOIN tuotetyyppi ON tuote.tyyppi_id=tuotetyyppi.id WHERE 1=1";
      if (nimi)
        sql = sql + " AND tuote.nimi like '" + nimi + "%'";
      if (valmistaja)
        sql = sql + " AND tuote.valmistaja like '" + valmistaja + "%'";
      if (tuotetyyppi && tuotetyyppi !== 0)
        sql = sql + " AND tuotetyyppi.id=" + tuotetyyppi;
      let query = sql;
      console.log(sql);
